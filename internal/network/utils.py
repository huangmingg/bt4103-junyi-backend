...
def create_graphing_dataframe(flp):
    ...
    # remove students who only took one exercise, since they do not contribute to edge weight
...
def get_true_edge(graph):
    ...
    average_score_from_src_to_tgt_dic = {(u, v): (dic['sum_of_correct_problems'] / dic['number_of_individual_students'])
                                         for u, v, dic in graph.edges(data=True)}
    nx.set_edge_attributes(graph, average_score_from_src_to_tgt_dic, 'average_score_from_src_to_tgt')

    # calculate the average performance of the target. handles part B
    def calc_average_performance_on_tgt(node):
        lst = list(map(lambda x: x[2], graph.in_edges(node, data='average_score_from_src_to_tgt')))
    ...
    nx.set_node_attributes(graph, average_performance_on_tgt, 'average_performance_tgt')

    # calculate the actual weight according to part C
    final_average_performance_dic = {(u, v): (w - graph.nodes[v]['average_performance_tgt']) for u, v, w in
                                     graph.edges(data='average_score_from_src_to_tgt')}
    nx.set_edge_attributes(graph, final_average_performance_dic, 'final_average_performance')
    return graph
...
def get_true_edge(graph):
    ...
    path_src_to_tgt_dic = {(u, v): 1 for u, v, dic in graph.edges(data=True)}
    nx.set_edge_attributes(graph, path_src_to_tgt_dic, 'shortest_path')
    ...
    average_performance_on_tgt = {node: calc_average_performance_on_tgt(node) for node in graph.nodes()}
    nx.set_node_attributes(graph, average_performance_on_tgt, 'average_performance_tgt')
...
def get_hubs_and_authorities(graph):
    ...
    iter_num = 1
    hits = False
    while not hits and iter_num <= 50:
        try:
            iter_num += 1
            max_iter += 100
            ...
            hits = nx.hits(graph,
                           max_iter=max_iter)  # https://stackoverflow.com/questions/63026282/error-power-iteration-failed-to-converge-within-100-iterations-when-i-tried-t
        except nx.PowerIterationFailedConvergence:
            continue
    if not hits:
        print(f'Hits not found for graph of num_node: {len(graph)}, num_edges: {len(graph.edges)}')
        # if hits did not converge, return back a dummy column (so it will be recommending a random exercise.. this)
        df = pd.DataFrame({'Authorities': [0] * len(graph), 'Hubs': [0] * len(graph)}, index=[node for node in graph.nodes()])
    else:
...
def calculate_path_weight(graph, path, method='final_average_performance'):
    """
    Parameters
    ----------
    graph: NetworkXGraph
    ...
    for u, v in zip(path[:-1], path[1:]):
        weight += graph[u][v][method]
    return weight


def k_shortest_paths(graph, source, target, num_paths=1, minimum_path_length=5, method='number_of_individual_students',
                     relax=True):
    paths_to_return = []
    # generator that recommends paths
    path_generator = nx.shortest_simple_paths(graph, source, target, weight=method)
    ...
    while len(paths_to_return) < num_paths:
        try:
            ...
            if method == "shortest_path":
                # if we want to find the shortest path, we should not care about the minimum path length
                paths_to_return.append(candidate_path)
            else:
...
def convert_edge_weights_for_dijstra(graph, method='number_of_individual_students'):
    # find the maximum weight for this 'method' to make our negated_weights all > 0
    offset = np.max(list(map(lambda x: x[2], graph.edges(data=method)))) + 1
    negated_weights = {(u, v): -w + offset for u, v, w in graph.edges(data=method)}
    # overwrite the original name with this new negated weights
    nx.set_edge_attributes(graph, negated_weights, method)
    return graph


def preprocess_edge_weights(graph, method):
    if method == 'shortest_path':
        ...
        graph = convert_edge_weights_for_dijstra(graph, method)
        return graph


def get_at_least_k_paths(graph, top_k_hubs, top_k_authorities, num_paths=3, minimum_path_length=5,
                         method='final_average_performance'):
    """
    Parameters
    ----------
    graph: NetworkXGraph
    ...
    graph = preprocess_edge_weights(graph, method)
            ...
            if hub != aut:
                try:
                    ...
                    if len(paths_to_return) < num_paths:
                        ...
                        paths = k_shortest_paths(graph, hub, aut, num_paths=2, minimum_path_length=minimum_path_length,
                                                 method=method)
...
def prune_by_quantile(graph, percentiles, criterion='final_average_performance', num_hubs_auth=10,
                      minimum_number_of_paths=5, minimum_path_length=5):
    """
    ...
    dictionary
     - {'Success': bool,
        ...
        'ErrorMessage': str}
    """
    attribute_data = pd.Series(map(lambda x: x[2], graph.edges(data=criterion)))
    best_length = -1
    best_quantile = -1
    best_paths = []
    for quantile in percentiles:
        ...
        F = slice_network_edges_by(graph, by=criterion, copy=True, lower_limit=attribute_data.quantile(quantile))
        try:
        ...
        paths = get_at_least_k_paths(F, top_k_hubs, top_k_authorities, num_paths=minimum_number_of_paths,
                                     minimum_path_length=minimum_path_length, method=criterion)
...
def slice_network_edges_by(graph, by, copy=False, lower_limit=None, upper_limit=None):
    """
    ...
    dup = graph.copy() if copy else graph
    if lower_limit:
        dup.remove_edges_from((u, v) for u, v, att in list(dup.edges(data=by)) if att <= lower_limit)
    elif upper_limit:
        dup.remove_edges_from((u, v) for u, v, att in list(dup.edges(data=by)) if att >= upper_limit)
    return dup


def tune_get_at_least_k_paths(graph, method='final_average_performance', num_hubs_auth=5, minimum_number_of_paths=5, minimum_path_length=5):
    """
    ...
    if no paths found:
      ...
      'success': False,
      'paths': res['paths'],
      'error_message': f'Cant find any path, returning the individual nodes.'
    }
    ...
    if at least some path is found:
      ...
      'success': True,
      'paths': res['paths'],
      'error_message': f'We can only return paths of minimum_length: {min_path_length}, and we can only find {len(res["paths"])} of them.'
      }
    ...
    if len(graph) == 2:
        ...
        paths = get_at_least_k_paths(graph, top_k_hubs, top_k_authorities, num_paths=1, minimum_path_length=0,
                                     method=method)
        return {
            'success': True,
            'paths': paths,
            'error_message': 'Length of graph is only 2, special case.'
            }

    # prune graph by the quantile and get the recommended path, lower bound is 25th percentile and the upper bound is 95th percentile
    res = prune_by_quantile(graph,
    ...
    # if cannot find the desired path, try again on the quantile value that gave us the best result so far (in terms of number of paths)
        ...
        iter = 0
        while not res['success'] and iter <= 2:
            iter += 1
            delta = 0.2
            # delta- range from the best _quantile, lower bound is 25th percentile and the upper bound is 95th percentile
            range_ = np.hstack((res['best_quantile'], np.linspace(max(0.1, res['best_quantile'] - delta),
                                                                  min(0.99, res['best_quantile'] + delta), 10)))[::-1]
        ...
        # if not found, return dictionary accordingly
            ...
            min_path_length = min([len(path) for path in res['paths']]) if len(res['paths']) > 0 else 0

            # if we cant find any path, then we return success = False
            if min_path_length == 0:
                res['paths'] = [(node, 0) for node in graph.nodes()]  # return individual exercises if no paths are found
                return {'success': False,
                        'paths': res['paths'],
                        'error_message': f'Cant find any path, returning the individual nodes.'}
            else:
...
def run_pipeline(log_problem):
    ...
    student_upid_frequencies = log_problem.groupby(['uuid', 'ucid'])['upid'].count()
    flp = lower_bound_student_exercise_frequencies(log_problem, lower_bound=student_upid_frequencies.quantile(0.05)) if len(
        log_problem['ucid'].unique()) != 1 else log_problem
...
def create_recommend_learning_paths(logs
                                    , num_paths=5
                                    , min_path_length=3
                                    , method="number_of_individual_students"
                                    ):
    """
    ...
    method: string
      The type of edge weight policy to be computed - currently supports ["number_of_individual_students", "final_average_performance", "shortest_path"]

    Returns
    -------
    paths: list of tuple
      returns the list of recommend path
    """
    logs = pd.DataFrame(logs)
    graph = run_pipeline(logs)

    if len(graph) <= min_path_length:
        return []
    res = tune_get_at_least_k_paths(graph, method=method, minimum_number_of_paths=num_paths, minimum_path_length=min_path_length)
    return res['paths']
